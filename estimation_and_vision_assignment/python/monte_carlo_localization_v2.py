...
class ParticleFilter(object):
    ...
    def handle_observation(self, laser_scan, dt):
        ...
        b = max(errors)
        b_minus_error = [(b-x) for x in errors]
        exp_error = [exp(error) for error in b_minus_error]

        # Make it a probability
        self.weights = [i/sum(exp_error) for i in exp_error]

        # Compute an estimate f the effective number of particles
        ...
        N_thresh = 0.25*len(self.weights)
        if (N_eff > N_thresh):
            self.resample()
    ...
    def resample(self):
        ...
        # random sample a point in the first arc range
        r = np.random.uniform(0, 1.0 / len(self.particles))
        idx = 0
        c = self.weights[0]  # Accumulated weight
        for i in range(len(self.particles)):

            # for each spoke
            u = r + (i / float(len(self.particles))) # The offset of the each spoke on the wheel

            # We know the location of the spoke and we want to check what partical is at this position by accumulate the weight see if 
            # we are reaching the u or not.
            while u > c:
                idx = idx + 1 # This will never out of range since we are sampling len(particles) times
                c = c + self.weights[idx]

            # We find the idex where the spoke is located, then we create the new partical and append it to the list
            p = self.particles[idx]
            new_particle = Particle(p.id,p.x, p.y, p.theta)
            sampled_particles.append(new_particle)
    ...
    def simulate_laser_scan_for_particle(self, x, y, yaw_in_map, angles, min_range, max_range):
        ...
        for angle in angles:
            ...
            range_curr = min_range
            # 2. Do ray tracing from (x,y) along the abosulte angle using step size range_step 
            while range_curr <= max_range:

                x_curr = x + range_curr*cos(abs_angle)
                y_curr = y + range_curr*sin(abs_angle)


                # If the currently examined point is outside the bounds of the workspace
                # stop if it reaches max_range
                if  x_curr > self.xmax or x_curr < self.xmin or y_curr > self.ymax or y_curr < self.ymin:
                    range_curr = max_range
                    break

                # If the currently examined point is within the bounds of the workspace
                # stop if it meets an obstacle or if it reaches max_range
                row, col = self.metric_to_grid_coords(x_curr, y_curr)
                if not self.grid_bin[row, col].all():
                    break

                range_curr += range_step
    ...
    def get_prediction_error_squared(self, laser_scan_msg, particle):
       ...
       # TODO: subsample the recived actual laser scan using the subsample_laser_scan method above
       actual_ranges, angles_in_upwards_baselaser_frame = self.subsample_laser_scan(laser_scan_msg)
       
       # TODO: simulate a laser scan using one of the methods of this class
       ranges = self.simulate_laser_scan_for_particle(particle.x, particle.y, particle.theta, angles_in_upwards_baselaser_frame, laser_scan_msg.range_min, laser_scan_msg.range_max)
       
       # TODO: compute the difference bwteen predicted ranges and actual ranges
       # Take the squared norm of that difference
       acc_error = 0
       for i in range(len(ranges)):
           acc_error += (ranges[i] - actual_ranges[i])**2
       return acc_error**0.5
